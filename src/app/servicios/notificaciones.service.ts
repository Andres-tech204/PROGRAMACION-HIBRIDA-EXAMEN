...
export class NotificacionesService {
  ...
  async AgregarNotificacion(n:Notificacion){
    const sql = `INSERT INTO ${this.TABLE_NAME}(Titulo, imagen, Descripcion, fecha) VALUES (?,?,?,Date('now'))`
    await this.db.run(sql, [n.Titulo, n.imagen, n.Descripcion]) 

  }

  async showNotificaciones():Promise<Notificacion[]> { 
    const sql = `SELECT * FROM ${this.TABLE_NAME}`
  const resultado = await this.db.query(sql)
  return resultado?.values ?? []
}

  async borrarNotificaciones(notificacionesID: number) {
    const sql = `DELETE FROM ${this.TABLE_NAME} WHERE ID =?`
    await this.db.run(sql, [notificacionesID])
  }
}